/*
  ...
  - A unique constraint covering the columns `[user_id]` on the table `user_bank` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `nama` to the `user_bank` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `user_bank` ADD COLUMN `nama` VARCHAR(200) NOT NULL,
    ADD COLUMN `status` VARCHAR(10) NOT NULL DEFAULT 'PENDING',
    MODIFY `account_id` VARCHAR(200) NOT NULL DEFAULT '-',
...
-- CreateIndex
CREATE UNIQUE INDEX `user_bank_user_id_key` ON `user_bank`(`user_id`);
