/*
  ...
  - You are about to drop the column `mesjid_id` on the `kas` table. All the data in the column will be lost.
  ...
  - You are about to alter the column `created_at` on the `kas_arus_detail` table. The data in that column could be lost. The data in that column will be cast from `DateTime(0)` to `DateTime`.
  - You are about to alter the column `updated_at` on the `kas_arus_detail` table. The data in that column could be lost. The data in that column will be cast from `DateTime(0)` to `DateTime`.
  ...
  - Added the required column `mesjid_user_id` to the `kas` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `kas` DROP COLUMN `mesjid_id`,
    ADD COLUMN `mesjid_user_id` VARCHAR(191) NOT NULL,
...
-- AlterTable
ALTER TABLE `kas_arus_detail` MODIFY `created_at` DATETIME NOT NULL DEFAULT NOW(),
    MODIFY `updated_at` DATETIME NULL DEFAULT NOW() ON UPDATE NOW();
